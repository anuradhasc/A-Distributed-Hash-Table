...
public class MainClient3{
 ...
 in = new BufferedReader(new InputStreamReader(indexSocket.getInputStream()));
	...
	System.out.println("Time after 100k put:"+ dateFormat.format(date1));
	 ...
	 Date date3 = new Date();
	 System.out.println("Time before 100k get:"+ dateFormat.format(date3));
	 for(int i = 0; i<100000; i++)
	 {
	 hashvalue = hf.HashFunc(Testkeyarr[i]);
	 Serverloc = hashvalue % noOfServers;
...
in = new BufferedReader(new InputStreamReader(indexSocket.getInputStream()));
	...
	Date date4 = new Date();
	System.out.println("Time after 100k get:"+ dateFormat.format(date4));

	 Date date5 = new Date();
	System.out.println("Time before 100k del:"+ dateFormat.format(date5));
	for(int i = 0; i<100000; i++)
	{
	hashvalue = hf.HashFunc(Testkeyarr[i]);
	Serverloc = hashvalue % noOfServers;
...
in = new BufferedReader(new InputStreamReader(indexSocket.getInputStream()));
	...
	Date date6 = new Date();
	System.out.println("Time after 100k del:"+ dateFormat.format(date6));
